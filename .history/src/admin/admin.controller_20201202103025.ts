import { Controller, Post, Put, Param, Body, Get, ParseIntPipe, Query, Delete } from '@nestjs/common';
...
export class AdminController {
    ...
    async getList(): Promise<AdminDto[]> {
        return (await this.coreGateway
            .request<AdminDto>('admin-list'))
            .toClassList(AdminDto);
    }

    @Delete(':id')
    async delete(@Param('id', ParseIntPipe) id: number): Promise<AdminDto> {
        return (await this.coreGateway
            .request<AdminDto>('admin-delete', { id }))
            .toClass(AdminDto);
    }
}
