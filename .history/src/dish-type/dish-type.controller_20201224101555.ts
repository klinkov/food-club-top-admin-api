...
export class DishTypeController {
    ...
    async create(@Body() body: EditDishTypeDto): Promise<DishTypeDto> {
        console.log(body);
        const createdType = (await this.coreGateway.request<DishTypeDto>('menu-dish-type-create', body)).toClass(DishTypeDto);
        return createdType;
    }

    @Put(':id')
    async edit(@Param('id', ParseIntPipe) id: number, @Body() body: EditDishTypeDto) {
        const editedType = (await this.coreGateway.request<DishTypeDto>('menu-dish-type-edit', {
            id,
            ...body
        })).toClass(DishTypeDto);
        return editedType;
    ...
    async list() {
        const list = (await this.coreGateway
            .request<DishTypeDto[]>('menu-dish-type-list',))
    ...
    async getOne(@Param('id', ParseIntPipe) id: number): Promise<DishTypeDto> {
        ...
        const item = (await this.coreGateway
            .request<DishTypeDto>('menu-dish-type-one', { id }))
            .toClass(DishTypeDto);
        console.log(item);
        return item;
    }
