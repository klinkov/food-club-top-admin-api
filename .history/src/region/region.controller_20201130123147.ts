...
export class RegionController {
    ...
    @Get()
    async list(@Query('countryId') countryId: number) {
        const list = (await this.coreGateway.request<RegionDto[]>('region-list', { countryId: Number(countryId) })).toClassList(RegionDto);
        return list;
    }
}
