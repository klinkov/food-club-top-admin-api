...
export class RegionController {
    ...
    async create(@Body() body: RegionDto) {
        const createdRegion = (await this.coreGateway.request('create-city', body)).toClass(RegionDto);
        return createdRegion;
    }

    @Put(':id')
    async edit(@Param('id', ParseIntPipe) id: number, @Body() body: RegionDto) {
        const editedRegion = (await this.coreGateway.request<RegionDto>('edit-city', {
            id,
            ...body
        })).toClass(RegionDto);
        return editedRegion;
    ...
    @Get()
    async list() {
        const list = (await this.coreGateway.request<RegionDto[]>('city-list')).toClassList(RegionDto);
        return list;
    }
}
